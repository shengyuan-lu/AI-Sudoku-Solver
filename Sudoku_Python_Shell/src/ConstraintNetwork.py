import Variable
import Constraint
import SudokuBoard
from math import floor

"""
    CSP representation of the problem. Contains the variables, constraints, and
    many helpful accessors.
"""
class ConstraintNetwork:

    # ==================================================================
    # Constructors
    # ==================================================================


    def __init__ ( self, sboard = None ):
        self.constraints = [] # list of Constraint objects
        self.variables = [] # list of Variable objects

        # if there is a board (2-d array) pass in
        if sboard != None:
            board = sboard.board # 2-d array
            temp = [] # list of temp Variable objects
            value = 0

            for i in range(sboard.N):

                for j in range(sboard.N):

                    value = board[i][j]
                    domain = [] # all possible values

                    # if value is not assigned, we get all possible values in the domains
                    if value == 0:
                        d = 1

                        while d <= sboard.N:
                            domain.append(d)
                            d += 1
                    else:
                        domain.append(value) # otherwise, the domain can be only this value

                    block = int(((floor(i/sboard.p) * sboard.p) + floor(j/sboard.q))) # calculate which block this value is on

                    # domain = list of possible values
                    # i = row
                    # j = col
                    # block = block
                    temp.append(Variable.Variable(domain,i,j,block))

            # key: row, value: all variables in the row
            rows = dict()

            # key: col, value: ll variables in the col
            cols = dict()

            # key: block, value: all variables in the block
            blocks = dict()

            for v in temp:
                row = v.row
                col = v.col
                block = v.block

                # if key not exist in the dicts, initialize them with an empty list first
                if not (row in rows.keys()):
                    rows[row] = []

                if not (col in cols.keys()):
                    cols[col] = []

                if not (block in blocks.keys()):
                    blocks[block] = []

                rows[row].append(v)
                cols[col].append(v)
                blocks[block].append(v)

            # add a variable object to the self.variable list
            for v in temp:
                self.addVariable(v)

            # for each row, create a new constraint object, add all variables of that row to it
            # then add the constraint object to self.constraints
            for e in rows:
                c = Constraint.Constraint()

                for v in rows[e]:
                    c.addVariable(v)

                self.addConstraint(c)

            # for each column, create a new constraint object, add all variables of that column to it
            # then add the constraint object to self.constraints
            for e in cols:
                c = Constraint.Constraint()

                for v in cols[e]:
                    c.addVariable(v)

                self.addConstraint(c)

            # for each block, create a new constraint object, add all variables of that block to it
            # then add the constraint object to self.constraints
            for e in blocks:
                c = Constraint.Constraint()

                for v in blocks[e]:
                    c.addVariable(v)

                self.addConstraint(c)

    # ==================================================================
    # Modifiers
    # ==================================================================

    def addConstraint ( self, c ):
        if c not in self.constraints:
            self.constraints.append( c )


    # add a variable object to the self.variable list
    def addVariable ( self, v ):
        if v not in self.variables:
            self.variables.append( v )

    # ==================================================================
    # Accessors
    # ==================================================================

    def getConstraints ( self ):
        return self.constraints

    def getVariables ( self ):
        return self.variables

    # Returns all variables that share a constraint with v
    def getNeighborsOfVariable ( self, v ):
        neighbors = set()

        for c in self.constraints:
            if c.contains( v ):
                for x in c.vars:
                    neighbors.add( x )

        neighbors.remove( v )

        return list( neighbors ) # a list of variables that share the same constraint as v

    # Returns true if every constraint is consistent
    def isConsistent ( self ):
        for c in self.constraints:
            if not c.isConsistent():
                return False

        return True

    # Returns a list of constraints that contains v
    def getConstraintsContainingVariable ( self, v ):
        """
            @param v variable to check
            @return list of constraints that contains v
        """
        outList = [] # a list of Constraint objects

        for c in self.constraints:
            if c.contains( v ):
                outList.append( c )

        return outList

    """
        Returns the constraints that contain variables whose domains were
        modified since the last call to this method.

        After getting the constraints, it will reset each variable to
        unmodified

        Note* The first call to this method returns the constraints containing
        the initialized variables.
    """
    # return a list of modified Constraint objects
    # set everything to unmodified
    def getModifiedConstraints ( self ):
        mConstraints = []
        for c in self.constraints:
            if c.isModified():
                mConstraints.append( c )

        for v in self.variables:
            v.setModified( False )

        return mConstraints

    # ==================================================================
    # String Representation
    # ==================================================================

    def __str__ ( self ):
        output = str(len(self.variables)) + " Variables: {"
        delim = ""

        for v in self.variables:
            output += delim + v.name
            delim = ","
        output += "}"

        output += "\n" + str(len(self.constraints)) + " Constraints:"
        delim = "\n"
        for c in self.constraints:
            output += delim + str(c)

        output += "\n"
        for v in self.variables:
            output += str(v) + "\n"

        return output

    # ==================================================================
    # Sudoku Board Representation
    # ==================================================================

    # Turn the constraint network into a SudokuBoard object

    def toSudokuBoard ( self, p, q ):
        n = p*q
        board = [[ 0 for j in range( n )] for i in range( n )]
        row = 0
        col = 0

        for v in self.variables:
            board[row][col] = v.getAssignment()
            col += 1
            if col == n:
                col = 0
                row += 1

        return SudokuBoard.SudokuBoard( p, q, board = board )